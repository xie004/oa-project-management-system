...
from app.services.task_recognition import preview_results, preview_status, start_preview
...
@app.post("/api/tasks/recognition-preview")
def api_task_recognition_preview(_: dict[str, Any] = Depends(admin_from_request)) -> dict[str, Any]:
    return start_preview("manual")


@app.get("/api/tasks/recognition-preview-status")
def api_task_recognition_preview_status(
...
@app.get("/api/tasks/recognition-preview-results")
def api_task_recognition_preview_results(
    run_id: str | None = None,
    result_type: Literal["all", "new", "duplicate", "invalid", "match_change", "progress_change", "failed"] = "all",
    page: int = Query(default=1, ge=1),
    page_size: int = Query(default=30, ge=1, le=100),
    _: dict[str, Any] = Depends(admin_from_request),
) -> dict[str, Any]:
    return preview_results(run_id, result_type, page, page_size)
