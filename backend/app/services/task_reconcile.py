...
def _task_similarity(left: str, right: str) -> float:
    return task_similarity(left, right)
...
def _apply_signal(
    ...
    if document.get("knowledge_status") != "indexed":
        return False, False
    # Freshness is evaluated per task through status_as_of. A newer unrelated
    # report must not suppress an earlier explicit completion signal.
...
def reconcile_task_progress(trigger: str = "manual") -> dict[str, Any]:
    ...
    try:
        ...
        from app.services.observations import link_evidence, candidates
