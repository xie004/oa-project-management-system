...
def prepare_candidate(
    ...
    ambiguous = False
    has_tasks_table = conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='tasks'").fetchone()
    if entity_type == "task" and has_tasks_table:
        from app.services.task_matching import ranked_matches, safe_match
        officials = rows_to_dicts(conn.execute(
            "SELECT * FROM tasks WHERE COALESCE(is_archived,0)=0 AND task_kind IN ('baseline','confirmed_addition')"
        ).fetchall())
        task_candidate = {**payload, "title": title, "description": description}
        rankings = ranked_matches(task_candidate, officials)
        safe = safe_match(task_candidate, officials)
        similarity = float(rankings[0]["similarity"]) if rankings else 0.0
        ambiguous = bool(rankings and safe is None and similarity >= .78)
        matched = None
        if safe:
            entity_id = ensure_entity(conn, "task", int(safe["id"]), safe["title"])
            matched = row_to_dict(conn.execute("SELECT * FROM project_entities WHERE id=?", (entity_id,)).fetchone())
    else:
    ...
    if ambiguous:
        ...
        result["warnings"] = [*warnings, "存在相似任务，但目标不唯一或工作范围不一致，需人工确认"]
        return result
    ...
    if similarity >= (0.90 if entity_type == "task" else 0.93):
        config = ENTITY_TABLES[entity_type]
        record = row_to_dict(
            conn.execute(
