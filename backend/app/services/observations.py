...
from app.services.task_matching import compatible_scope, ranked_matches, safe_match, task_scope, task_similarity
...
def scope_signature(task):
    signature = task_scope(task)
    return signature["batches"], signature["phases"]


def similarity(left, right):
    return task_similarity(str(left or ""), str(right or ""))


def candidates(task, officials):
    result = ranked_matches(task, officials)[:5]
    for item in result:
        item["reason"] = "标准标题及工作范围相同" if item["exact"] else ("工作对象、动作和范围近似，需核对" if item["scopeCompatible"] else "批次、环境或测试阶段不一致，需逐项确认")
    return result
