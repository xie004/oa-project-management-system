          ...
          <tbody>
            {documents.map((doc) => (
              <tr key={doc.id}>
                ...
                <td>
                  {doc.recognition_rule_version ? (
                    <div className={`recognition-cell ${doc.recognition_failed_batches ? "has-warning" : ""}`}>
                      <div className="recognition-cell-head">
                        <span>{Math.round((doc.recognition_coverage || 0) * 100)}% 覆盖</span>
                        <small>{String(doc.recognition_rule_version).replace("task-recognition-", "")}</small>
                      </div>
                      <small>
                        有效 {doc.recognition_candidate_count || 0} · 过滤 {doc.recognition_rejected_count || 0}
                        {doc.recognition_failed_batches ? ` · 失败批次 ${doc.recognition_failed_batches}` : ""}
                      </small>
                      {!!doc.recognition_rejection_examples?.length && (
                        <details>
                          <summary>查看过滤原因</summary>
                          {doc.recognition_rejection_examples.map((item, index) => (
                            <p key={`${item.reason_code}-${index}`} title={item.evidence_text || item.title}>
                              {item.reason || item.reason_code}{item.locator ? `（${item.locator}）` : ""}
                            </p>
                          ))}
                        </details>
                      )}
                      {!!doc.recognition_report?.failedBatches?.length && (
                        <details className="error">
                          <summary>批次错误</summary>
                          {doc.recognition_report.failedBatches.slice(0, 3).map((item, index) => (
                            <p key={index}>{item.error || `批次 ${item.batchIndex ?? index + 1} 失败`}</p>
                          ))}
                        </details>
                      )}
                    </div>
                  ) : <span className="muted-inline">未运行新规则</span>}
                </td>
                <td>
...
function DeliverablesView({
  ...
  const ready = deliverables.filter((item) => !item.is_archived && ["finalized", "submitted"].includes(item.status)).length;
  const linked = deliverables.filter((item) => !item.is_archived && item.document_id).length;
            ...
            <tbody>
              {activeDeliverables.map((item) => (
                <tr key={item.id}>
                  ...
                  <td data-label="状态">
                    <select value={item.status} disabled={!isAdmin || showArchived} onChange={(e) => onPatch(item.id, { status: e.target.value })}>
                      {deliverableStatuses.map(([value, label]) => <option value={value} key={value}>{label}</option>)}
                    </select>
                  </td>
                  <td data-label="责任人">
                    <input value={item.owner || ""} readOnly />
                  </td>
                  <td data-label="计划日期">
                    <input type="date" value={item.planned_date || ""} readOnly />
                  </td>
                  <td data-label="提交日期">
                    <input type="date" value={item.submitted_date || ""} readOnly />
                  </td>
                  <td className="deliverable-file-cell" data-label="关联文件">
                    <select
                      value={item.document_id || ""}
                      disabled={!isAdmin || showArchived}
...
function App() {
  ...
  const refresh = async () => {
    ...
    const [dashboard, taskList, milestoneList, meetingList, changeList, documentList, deliverableList, suggestionList, wikiPageList, weeklyData, targetList, currentUploadStatus] =
      await Promise.all([
        ...
        api.get(auth.isAdmin ? "/api/deliverables?includeArchived=true" : "/api/deliverables"),
  ...
  useEffect(() => {
    if (auth.isAdmin) api.get("/api/deliverables?includeArchived=true").then(setDeliverables).catch(() => {});
  }, [auth.isAdmin]);
