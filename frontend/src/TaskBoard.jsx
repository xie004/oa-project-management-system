...
export default function TaskBoard({ api, tasks, onCreate, onPatch, onBulkPatch, onChanged, onReconcile, reconcileStatus, isAdmin, actionStates, onPreview, onEvidence, focusedTaskId, onFocusedTaskHandled }) {
  ...
  const bulk = async value => {
    ...
    try {
      ...
      setSelected(current => selectionAfterBatch(current, { updatedIds: [...(result.updatedIds || []), ...current.filter(id => !ids.includes(id))] }));
      setMessage(`已${value ? "加入" : "移出"} ${result.updated || 0} 条${failed.length ? `；${failed.length} 条未处理：${failed.map(item => `#${item.id} ${item.reason}`).join("；")}` : "，已清空勾选"}${value && hideAdded && !failed.length ? "；当前仅显示未进甘特图的任务" : ""}。任务记录仍保留。`);
      if (value && hideAdded && !failed.length) setGantt("out");
